package net.glasslauncher.mods.starlightbeta;
...
public class StarlightEngine {
    ...
    public static void initLightingForChunk(Chunk chunk) {
        ((StarlightWorld) chunk.world).starlight_Beta$getBlockLight().initBlockLight(chunk.x, chunk.z);

        if (!chunk.world.dimension.hasCeiling) {
            ((StarlightWorld) chunk.world).starlight_Beta$getSkyLight().initSkylight(chunk.x, chunk.z);
        }
    }
    ...
    protected final void setupCaches(final World chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final Chunk chunk = !chunkProvider.hasChunk(cx, cz) ? null : chunkProvider.getChunk(cx, cz);
    ...
    protected final ChunkNibbleArray getNibbleFromCache(final int chunkX, final int chunkZ) {
        final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
        return chunk == null ? null : (this.skylightPropagator ? chunk.skyLight : chunk.blockLight);
    }
    ...
    protected final int getLightLevel(final int worldX, final int worldY, final int worldZ) {
        final ChunkNibbleArray nibble = this.getNibbleFromCache(worldX >> 4, worldZ >> 4);
    ...
    protected final void performLightIncrease(final World world) {
        ...
        while (queueReadIndex < queueLength) {
            ...
            for (final AxisDirection propagate : checkDirections) {
                ...
                final ChunkNibbleArray currentNibble = this.getNibbleFromCache(offX >> 4, offZ >> 4);
                ...
                final int opacityCached = Block.BLOCKS_LIGHT_OPACITY[blockState];
    ...
    protected final void performLightDecrease(final World world) {
        ...
        while (queueReadIndex < queueLength) {
            ...
            for (final AxisDirection propagate : checkDirections) {
                ...
                long chunkHash = (offX + (offZ << 6) + (offY << 12) + encodeOffset) & ((1L << (6 + 6 + 16)) - 1);
                ...
                final int emittedLight = Block.BLOCKS_LIGHT_LUMINANCE[blockState] & emittedMask;
                if (emittedLight != 0) {
                    // re-propagate source
    ...
    public final void checkBlockEmittance(final int worldX, final int worldY, final int worldZ) {
        ...
        try {
            ...
            final int emittedLevel = Block.BLOCKS_LIGHT_LUMINANCE[blockState] & emittedMask;

            this.setLightLevel(worldX, worldY, worldZ, emittedLevel);
            // this accounts for change in emitted light that would cause an increase
            long chunkHash = (worldX + (worldZ << 6) + (worldY << (6 + 6)) + encodeOffset) & ((1L << (6 + 6 + 16)) - 1);
            if (emittedLevel != 0) {
    ...
    protected final int tryPropagateSkylight(final World world, final int worldX, int startY, final int worldZ) {
        ...
        for (;startY >= 0; --startY) {
            ...
            final int opacityIfCached = Block.BLOCKS_LIGHT_LUMINANCE[current];
    ...
    public final void initBlockLight(final int chunkX, final int chunkZ) {
        this.setupCaches(this.world, (chunkX << 4) | 7, 64, (chunkZ << 4) | 7, true);
        try {
            if (this.world.dimension.id == 0) { // isHell
                final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
                final ChunkNibbleArray nibble = this.getNibbleFromCache(chunkX, chunkZ);
                final int encodeOffset = this.coordinateOffset;
                ...
                for (int y = 0; y <= 127; ++y) {
                    for (int z = 0; z <= 15; ++z) {
                        for (int x = 0; x <= 15; ++x) {
                            ...
                            final int emittedLight = Block.BLOCKS_LIGHT_LUMINANCE[blockState];
    ...
    protected final void propagateNeighbourLevels(final int chunkX, final int chunkZ) {
        final ChunkNibbleArray currentNibble = this.getNibbleFromCache(chunkX, chunkZ);
        for (int currSectionY = (127 >> 4); currSectionY >= 0; --currSectionY) {
            for (final AxisDirection direction : ONLY_HORIZONTAL_DIRECTIONS) {
                ...
                final ChunkNibbleArray neighbourNibble = this.getNibbleFromCache(chunkX + neighbourOffX,
